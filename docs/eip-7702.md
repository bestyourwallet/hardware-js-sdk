...
- **用户体验**：连续两次出现升级界面（这里目前是有问题的，需要 metamask 进行处理，具体上下文：https://ukeygroup.slack.com/archives/C04596CP3BN/p1756293294921909
...
**2. 固件层面的验证**：
```python
# core/src/apps/ethereum/ukey/sign_tx_eip7702.py
...
- 白名单强制校验（仅允许已注册的委托合约地址）
  - 作用：降低“授权到恶意委托合约”的风险
  - 代码：core/src/apps/ethereum/ukey/eip7702_delegators.py（白名单表与初始 data 规则）；core/src/apps/ethereum/ukey/sign_tx_eip7702.py 中 validate_delegate
...
- 授权列表长度限制
  ...
  - 代码：core/src/apps/ethereum/ukey/sign_tx_eip7702.py → check_common_fields
...
- 撤销（Revoke）支持
  ...
  - 代码：core/src/apps/ethereum/ukey/eip7702_delegators.py（Revoke 条目）
